function [contour,nbpoints]=tortue(img)
% 1. Initialisation
...
% 5. Sélectionner le voisin q de p dans la direction_courante
% indiquée par le code de freeman - on suppose qu'il existe
boucle=0;
while (~boucle)
   boucle=1;
   voisin=0;
   while (~voisin)
      ...
      %d'un pixel blanc (connexité 4 V) ou s'il a deja ete identifie comme
      %un point du contour alors
      if (double(img(q(1)-1,q(2)))+...
              double(img(q(1)+1,q(2)))+...
              double(img(q(1),q(2)-1))+...
              double(img(q(1),q(2)+1))==0) ...
              | (img(q(1),q(2))~=0) ...
              | (imgcontour(q(1),q(2))==128)
       
         % Si ce n'est le point de depart alors changement de direction et 
         %retour en 5
